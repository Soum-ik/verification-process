...
//             </div>}
//             <div onClick={goBack()} className="w-[360px] pt-[20px] flex items-end justify-end">
//                 <div>
...
const FaceVerification: React.FC = () => {
    const {
        videoRef,
        canvasRef,
        currentStep, isLoading, modelLoaded, hasMovedLeft, hasMovedRight, bgVideoRef
    } = useFaceRecognition();
    ...
    useEffect(() => {
        if (currentStep === 4) {
            const timer = setTimeout(() => {
                navigate('/capture-passport'); // Replace with your target route
    ...
    return (
        <div className="flex min-h-[65vh] w-full items-center justify-center flex-col mx-auto p-[20px]">
            ...
            <video
                className=' absolute top-0 w-full h-full z-10 opacity-20 object-cover'
                ref={bgVideoRef}
                autoPlay
                muted
                playsInline
            ...
            {(modelLoaded && currentStep >= 1) && <div className='w-full  h-full flex justify-center items-center mt-[60px]'>
                <img src={gif[currentStep]} className='gif-size' alt="" />
            </div>}
            <div className="fixed mt-[650px] bg-white p-4 flex items-start justify-start mx-auto w-[360px]">
                <button className="w-[360px] cursor-default z-50 mx-auto rounded-md bg-[rgba(21,21,21,0.50)] px-[12px] py-[10px] text-center font-Inter text-[14px] font-medium leading-[18px] text-[#fff]">
